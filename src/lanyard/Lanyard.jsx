...
function preparePhotoTexture(photoTexture, targetAspect) {
  if (!photoTexture?.image) return photoTexture;
  ...
  if (imageAspect > targetAspect) {
    ...
    photoTexture.repeat.set(repeatX, 1);
    photoTexture.offset.set((1 - repeatX) / 2, 0);
  } else {
    const repeatY = imageAspect / targetAspect;
    photoTexture.repeat.set(1, repeatY);
    photoTexture.offset.set(0, (1 - repeatY) / 2);
  }
  ...
  return (
    <div ref={wrapperRef} className="lanyard-wrapper">
      ...
      >
        ...
        <pointLight castShadow={!isMobile} position={[3, 1, 5]} intensity={10} distance={14} color="#8bf4e9" />
        <pointLight position={[-3, -2, 4]} intensity={8} distance={12} color="#9c8cff" />
...
function Band({ isMobile, frontImage, backImage, imageFit, lanyardImage, lanyardWidth }) {
  ...
  const directPhotoAspect = 0.7164179086;
  const frontPhotoTexture = useMemo(() => preparePhotoTexture(frontTex, directPhotoAspect), [frontTex]);
  const backPhotoTexture = useMemo(() => preparePhotoTexture(backTex, directPhotoAspect), [backTex]);
  ...
  return (
    <>
        ...
        <RigidBody position={[2, 0, 0]} ref={card} {...segmentProps} type={dragged ? "kinematicPosition" : "dynamic"}>
          ...
          >
            <mesh geometry={nodes.card.geometry} castShadow receiveShadow>
              <meshPhysicalMaterial color="#152530" clearcoat={isMobile ? 0.2 : 0.65} clearcoatRoughness={0.2} roughness={0.5} metalness={0.18} />
            </mesh>
            <mesh position={[0, 0.523, 0.014]}>
              <planeGeometry args={[0.68, 0.95]} />
              <meshBasicMaterial map={frontPhotoTexture} toneMapped={false} side={THREE.DoubleSide} />
            </mesh>
            <mesh position={[0, 0.523, -0.008]} rotation={[0, Math.PI, 0]}>
              <planeGeometry args={[0.68, 0.95]} />
              <meshBasicMaterial map={backPhotoTexture} toneMapped={false} side={THREE.DoubleSide} />
            </mesh>
      ...
      <mesh ref={band}>
        <meshLineGeometry />
        <meshLineMaterial color="white" depthTest={false} resolution={isMobile ? [720, 1280] : [1000, 1000]} useMap map={texture} repeat={[-4, 1]} lineWidth={lanyardWidth} />
      </mesh>
    </>
  );
}
